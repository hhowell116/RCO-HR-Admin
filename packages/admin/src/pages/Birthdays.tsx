...
export function Birthdays() {
  ...
  const birthdays = useMemo(() => {
    return employees
      .filter((e) => e.isActive && e.birthMonth === viewMonth.month)
  ...
  return (
    <div>
      <div className="mb-6">
        <h2 className="text-2xl font-serif font-bold text-brand-deep-brown">Birthdays</h2>
        <p className="text-sm text-brand-taupe mt-0.5">Employee birthdays by month</p>
      </div>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-xl border border-brand-border p-8 text-center">
          <Cake size={32} className="text-brand-light-gray mx-auto mb-2" />
          <p className="text-sm text-brand-taupe">No birthdays in {MONTHS[viewMonth.month - 1]}.</p>
        </div>
      ) : (
            ...
            >
              ...
              <div>
                ...
                <p className="text-xs text-brand-taupe">
                  {MONTHS[emp.birthMonth - 1]} {emp.birthDay}
                </p>
                {emp.department && (
